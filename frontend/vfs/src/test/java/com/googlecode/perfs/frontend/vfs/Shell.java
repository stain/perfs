...
public class Shell {
	...
	private void help() {
		...
		System.out
				.println("ls [-R] [path]     Lists contents of a file or folder.");
		System.out.println("pwd                Displays current folder.");
		System.out.println("rm <path>          Deletes a file or folder.");
		System.out
				.println("touch <path>       Sets the last-modified time of a file.");
	...
	/**
	 * Does a 'cd' command. If the taget directory does not exist, a message is
	...
	private void ls(final String[] cmd) throws FileSystemException {
		...
		if (file.getType() == FileType.FOLDER) {
			...
			final String lastMod = dateFormat.format(new Date(content
					.getLastModifiedTime()));
	 ...
	 */
	private void listChildren(final FileObject dir, final boolean recursive,
